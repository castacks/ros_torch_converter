...
def get_ns_in_sec(): # TODO: move to utils
    return rclpy.clock.Clock().now().to_msg().sec + rclpy.clock.Clock().now().to_msg().nanosec * 1e-9
...
class ROSTorchConverter(Node):
    ...
    def setup_subscribers(self):
        for topic_conf in self.config['topics']:
            ...
            sub = self.create_subscription(
                    ...
                    lambda msg, topic_conf=topic_conf: self.handle_msg(msg, topic_conf),  # Callback with additional args
    ...
    def can_get_data(self):
        return all([get_ns_in_sec()- dt < self.config['max_age'] for dt in self.data_times.values()])
    ...
    def get_status_str(self):
        ...
        for topic_conf in self.config['topics']:
            ...
            out += '\t{:<16} exists: {} age:{:.2f}s\n'.format(topic_conf['name']+ ' ' + topic_conf['topic'] +':', data_exists, data_age)
